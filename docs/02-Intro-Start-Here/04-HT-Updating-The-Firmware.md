...
misconfiguration.  A guide to downloading gerefi firmware and setting up TunerStudio can be found
...
refer to the same thing: taking the compiled gerefi firmware and loading it into the non-volatile flash memory on the ECU.

## Updating the firmware with gerefi console

After confirming the firmware to flash to the ECU, software to do so must be used.  gerefi provides a utility, the **gerefi
console**, which is capable of flashing firmware to supported ECUs and is the recommended way to update gerefi firmware.
The gerefi console is included with each firmware release, and bundles can be downloaded from [the release
page](https://github.com/gerefi/ecu-fw/releases).

After extracting the bundle contents, navigate to the `console/` directory and run the gerefi console program (e.g.
`ecu_console.exe` if using Windows).  After first confirming **TunerStudio is not running**, connect the ECU to the
computer.  gerefi console will automatically detect the board and present similar to the display below.
...
In the event of a firmware update/flash failure, or some other firmware corruption issue, such that gerefi console nor
TunerStudio recognize the ECU when connected, the board needs to be first forced into bootloader mode before updating.
In these situations, the gerefi console cannot communicate with the firmware and so is unable to configure it into
...
When a board has been successfully booted into bootloader/DFU mode, the gerefi console will recognize this and display the
